...
import (
	...
	web "github.com/homeric-io/tinm/tinm/http"
	"github.com/homeric-io/tinm/tinm/rpc"
	"github.com/homeric-io/tinm/tinm/server"
	"github.com/homeric-io/tinm/tinm/sign"
	"github.com/homeric-io/tinm/tinm/storage"
	"github.com/homeric-io/tinm/tinm/tlsutil"
	"github.com/homeric-io/tinm/tinm/version"
...
func main() {
	...
	flag.StringVar(&flags.dataPath, "data-path", "/var/lib/tinm", "Path to data directory")
	flag.StringVar(&flags.assetsPath, "assets-path", "/var/lib/tinm/assets", "Path to static assets")
	...
	// gRPC Server TLS
	flag.StringVar(&flags.grpcCertFile, "cert-file", "/etc/tinm/server.crt", "Path to the server TLS certificate file")
	flag.StringVar(&flags.grpcKeyFile, "key-file", "/etc/tinm/server.key", "Path to the server TLS key file")

	// gRPC TLS Client Authentication
	flag.StringVar(&flags.grpcCAFile, "ca-file", "/etc/tinm/ca.crt", "Path to the CA verify and authenticate client certificates")

	// Signing
	flag.StringVar(&flags.keyRingPath, "key-ring-path", "", "Path to a private keyring file")

	// SSL flags
	flag.StringVar(&flags.tlsCertFile, "web-cert-file", "/etc/tinm/ssl/server.crt", "Path to the server TLS certificate file")
	flag.StringVar(&flags.tlsKeyFile, "web-key-file", "/etc/tinm/ssl/server.key", "Path to the server TLS key file")
	...
	flag.Parse()
	if err := SetFlagsFromEnv(flag.CommandLine, "tinm"); err != nil {
		log.Fatal(err.Error())
	}
	// restrict OpenPGP passphrase to pass via environment variable only
	passphrase := os.Getenv("tinm_PASSPHRASE")
	...
	if flags.rpcAddress != "" {
		log.Infof("Starting tinm gRPC server on %s", flags.rpcAddress)
	...
	if flags.tlsEnabled {
		// HTTPS Server
		log.Infof("Starting tinm HTTPS server on %s", flags.address)
		...
		// HTTP Server
		log.Infof("Starting tinm HTTP server on %s", flags.address)
