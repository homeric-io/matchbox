...
import (
	...
	"github.com/homeric-io/tinm/tinm/client"
...
var (
	...
	RootCmd = &cobra.Command{
		Use:   "bootcmd",
		Short: "A command line client for the tinm service.",
		Long: `A CLI for the tinm Service
...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&globalFlags.caFile, "ca-file", "/etc/tinm/ca.crt", "Path to the CA bundle to verify certificates of TLS servers")
	// gRPC TLS Client Authentication
	RootCmd.PersistentFlags().StringVar(&globalFlags.certFile, "cert-file", "/etc/tinm/client.crt", "Path to the client TLS certificate file")
	RootCmd.PersistentFlags().StringVar(&globalFlags.keyFile, "key-file", "/etc/tinm/client.key", "Path to the client TLS key file")
